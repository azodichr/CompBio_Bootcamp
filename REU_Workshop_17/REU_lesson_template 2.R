...
# Before we begin, we need to filter our data for expression > 0
circadian_nonzero = circadian_data[circadian_data[,2]>0,]
...
log_division = function(num,div){return(log(num/div))}

fold_change = function(expression_vector){
  fc_values <- apply(as.matrix(expression_vector),1,log_division,div=expression_vector[1])
  return(fc_values)
}
...
average_matrix <- apply(circadian_nonzero[,-1],1,average_reps)
...
fc_df$Gene <- circadian_nonzero$Gene

# Finally, do this for all changes
# This is the slow, simple way
fc_matrix <- matrix(,nrow=dim(circadian_nonzero)[1],ncol=6)
for (index in seq(1,dim(circadian_nonzero)[1])){
  row_average <- average_reps(circadian_nonzero[index,-1])
...
fc_df <- fc_df[is.finite(rowSums(fc_df[,0:6])),]
fc_df_c <- fc_df[,-7]
rownames(fc_df_c) <- fc_df[,7] 

#generate cluster with kmeans
k <- 10
c <- kmeans(fc_df_c, k, iter.max = 100, nstart = 50)
c$size
...
agg$Row.names <-NULL
agg$V1 <- NULL
names(agg) <- c('cluster', 1, 2, 3, 4, 5, 6)
...
agg_m$cluster <- as.factor(agg_m$cluster)
agg_m$variable <- as.numeric(agg_m$variable)
g <- ggplot(agg_m, aes(x=variable, y=value, color=cluster))
g + geom_line() +
  ylim(-10,10)
